...
import { projectExperienceFormSchemaExcludeHidden } from '@/lib/schema/resume/projectExperience'
...
function ProjectExperienceForm({ className }: { className?: string }) {
  ...
  const form = useForm<ProjectExperienceFormExcludeHidden>({
    ...
    defaultValues: {
      ...
      participantRole: projectExperience.participantRole,
      projectDuration: projectExperience.projectDuration,
      projectInfo: projectExperience.projectInfo,
    },
    mode: 'onChange',
  ...
  useEffect(() => {
    const subscription = form.watch((value, { name }) => {
      ...
      updateForm('projectExperience', value)
    })
  ...
  return (
    <Form {...form}>
      <form id="project-experience-form" className={cn(className)}>
        ...
        <div className="mt-4 space-y-4 w-full">
          {fields.map((item, index) => (
            ...
            >
              <FormField
                name={`projectInfo.${index}.content`}
                control={form.control}
                render={({ field }) => (
                  <FormItem>
                    <FormControl>
