      ...
      <DrawerDescription asChild>
        <div className='text-sm text-muted-foreground flex flex-wrap items-center gap-3'>
          ...
          <div className='flex items-center gap-2'>
            ...
            {isSharing && <span className='text-xs font-medium text-emerald-600'>协作人数 {participantCount}</span>}
          </div>
        </div>
      </DrawerDescription>
    </DrawerHeader>
...
function renderSyncStatus({
  ...
  if (lastSyncTime) {
    return (
      <span className='text-sm text-green-600 font-normal'>已同步 {new Date(lastSyncTime).toLocaleTimeString()}</span>
    )
  }
