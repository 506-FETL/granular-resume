 ...
 */
export function removeEmptyParagraphs(content: JSONContent) {
  return {
    ...content,
 ...
 */
export function isSelectionValid(editor: Editor | null, selection?: Selection, excludedNodeTypes: string[] = ['imageUpload', 'horizontalRule']): boolean {
  if (!editor)
    return false
  if (!selection)
